...
│   └── wrestling.ts        # Борьба (TODO)
│
├── athletics/               # Легкая атлетика
│   ├── sprint.ts           # Спринт (13 упражнений)
│   ├── middle-distance.ts  # Средние дистанции (TODO)
│   └── long-distance.ts    # Длинные дистанции (TODO)
│
├── aquatic/                 # Водные виды
│   └── swimming.ts         # Плавание (TODO)
│
├── cycling/                 # Велоспорт
│   ├── track.ts            # Трек (TODO)
│   └── road.ts             # Шоссе (TODO)
│
├── rowing/                  # Гребля
│   └── rowing.ts           # Академическая гребля (TODO)
│
├── gymnastics/              # Гимнастика
│   └── artistic.ts         # Спортивная гимнастика (TODO)
│
...
- Спортзал (GYM): 116
  ...
  - Пресс: 12
  - Кардио: 20
- Единоборства (COMBAT): 7
  - Бокс: 7
- Легкая атлетика (ATHLETICS): 13
  - Спринт: 13

**Примечание:** Дополнительно есть 14 упражнений из спринта и бокса, которые можно отфильтровать как функциональные/HIIT/плиометрические через систему тегов.
